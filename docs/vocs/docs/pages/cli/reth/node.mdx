...
Engine:
      ...
      --engine.memory-block-buffer-target <MEMORY_BLOCK_BUFFER_TARGET>
          ...
          If omitted, this defaults to the lesser of `--engine.persistence-threshold` and the configured default memory block buffer target.
      ...
      --engine.sender-recovery-cache
          Enable caching recovered transaction senders across transaction ingress and payload execution
      ...
      --engine.suppress-persistence-during-build
          ...
          When enabled, persistence cycles are deferred while a payload build is active. Useful on chains with short block times where persistence I/O can interfere with block building latency.
...
JIT:
      ...
      --jit.debug
          ...
          IR, assembly, and bytecode are written to `<datadir>/jit/<spec_id>/<code_hash>/` for each compiled contract. Note that this is not ever cleaned up, and has a non negligible performance overhead.
