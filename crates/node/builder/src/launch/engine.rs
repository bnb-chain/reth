            ...
            + EngineValidatorAddOn<NodeAdapter<T, CB::Components>>,
    {
        ...
        ctx.task_executor().spawn_critical_with_graceful_shutdown_signal(
            ...
            move |graceful| async move {
                ...
                let _ = exit.send(res);
                // Note: `_guard` (the GracefulShutdownGuard) drops at `break` inside the
                // graceful arm above, decrementing TaskManager's graceful_tasks counter and
                // allowing graceful_shutdown_with_timeout to return. If the loop exits via
                // any other arm (stream exhausted, FatalError, backfill complete), the
                // graceful arm never fired and no guard exists.
            },
        );
