        ...
        + EngineValidatorAddOn<NodeAdapter<T, CB::Components>>,
{
    ...
    async fn launch_node(
        ...
        let engine_tree_config = if is_triedb_active() &&
            (engine_tree_config.persistence_threshold() != 32 ||
                engine_tree_config.memory_block_buffer_target() != 16)
        {
            info!(
                target: "reth::cli",
                "TrieDB is active, setting persistence threshold to 32 and memory block buffer target to 16, old_threshold={}, old_target={}",
                engine_tree_config.persistence_threshold(),
                engine_tree_config.memory_block_buffer_target()
            );
            engine_tree_config
                .with_persistence_threshold(32)
                .with_memory_block_buffer_target(16)
        } else {
