...
/// Shared engine [`Runtime`] published by the engine tree at validator construction so that
/// embedders running their own payload pipeline (e.g. a custom miner) can submit proof work to
/// the SAME rayon pools instead of constructing a second `Runtime` — which would create a
