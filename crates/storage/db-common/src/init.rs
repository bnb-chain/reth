    ...
    PF::ChainSpec: EthChainSpec<Header = <PF::Primitives as NodePrimitives>::BlockHeader>,
{
    ...
    if !provider_rw.is_active_eth_triedb() {
...
where
    Provider: StaticFileProviderFactory
        ...
        + AsRef<Provider>,
{
    if provider.is_active_eth_triedb() {
        insert_eth_triedb_state(provider, alloc, 0)
    } else {
...
pub fn insert_eth_triedb_state<'a, 'b, Provider>(
    provider: &Provider,
    alloc: impl Iterator<Item = (&'a Address, &'b GenesisAccount)>,
    block: u64,
) -> ProviderResult<()>
