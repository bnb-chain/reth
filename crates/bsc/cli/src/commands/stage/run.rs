...
impl<C: ChainSpecParser<ChainSpec = BscChainSpec>> Command<C> {
        ...
        N: NodeTypesWithEngine<ChainSpec = C::ChainSpec>,
    {
        ...
        let (mut exec_stage, mut unwind_stage): (Box<dyn Stage<_>>, Option<Box<dyn Stage<_>>>) =
            match self.stage {
                StageEnum::Headers => {
                    let chain_spec = provider_factory.chain_spec().inner.clone();
                    let consensus =
                        Arc::new(Parlia::new(Arc::new(chain_spec), config.parlia.clone()));
                ...
                StageEnum::Execution => {
                    ...
                    let executor_provider =
                        BscExecutorProvider::bsc(Arc::new(chain_spec), provider_factory.clone());
                    (
