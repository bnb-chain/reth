...
pub enum RlpRawError {
    ...
    ValueTooLarge,
    /// Expected an RLP list but found a different kind.
    ExpectedList,
    /// The RLP value claims to be larger than the provided buffer.
    UnexpectedList,
}
...
impl fmt::Display for RlpRawError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            RlpRawError::ExpectedList => write!(f, "expected list"),
            RlpRawError::UnexpectedList => write!(f, "unexpected list"),
        }
    }
}
...
/// Splits `b` assuming it starts with an RLP list and returns the list content and the rest.
///
...
/// Counts the number of RLP-encoded values contained in `b`.
///
/// Mirrors go-ethereum's `CountValues` by iteratively calling `read_kind` and
/// advancing by the tag and content sizes until the buffer is exhausted.
pub fn count_values(mut b: &[u8]) -> Result<usize, RlpRawError> {
...
/// Splits `b` into the content of an RLP string and any remaining bytes after the string.
///
