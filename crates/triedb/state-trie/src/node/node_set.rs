...
impl NodeSet {
    ...
    pub fn add_node(&mut self, path: &[u8], node: TrieNode) {
        ...
        // Check if we're overwriting an existing node
        if let Some(existing_node) = self.nodes.get(&path_str) {
            // Overwrite happens, revoke the counter for the existing node
            if existing_node.is_deleted() {
                self.deletes -= 1;
            } else {
                self.updates -= 1;
            }
